class EloACL:
    ...
    def print_acl(self, file_id, current_acl, phase="Vor dem Setzen"):
        if current_acl:
            ...
            print(f"❌ Keine ACL-Einträge für Datei {file_id} vorhanden.")
    ...
    def set_read_only(self, file_id, filename):
        ...
        print(f"📄 API-Antwort für Datei {file_id}: {file_info}")
        print(f"📄 API-Antwort für Datei {file_id}: {file_info2}")

        attr = file_info2.get('fields', {})
        exported_flag = attr.get('EXPORTED')

        if exported_flag != '1':
            print(f"⚠️ Datei {file_id} hat nicht das Attribut 'EXPORTED:1' → wird jetzt gesetzt...")
        ...
        if exported_flag == '1':
            ...
            if current_acl:
                ...
                if response is not None:
                    print(f"✅ Schreibrechte für Datei {file_id} entfernt. Andere Rechte wurden beibehalten.")
                else:
                    print(f"❌ Fehler beim Setzen der ACL für Datei {file_id}.")

                updated_acl = self.client.get(f"/api/files/{file_id}/acl")
                self.print_acl(file_id, updated_acl, "Nach dem Setzen")
            else:
                print(f"❌ Keine ACL-Einträge für Datei {file_id} gefunden.")
        else:
            print(f"❌ Datei {file_id} hat immer noch nicht das Attribut 'EXPORTED:1'")
